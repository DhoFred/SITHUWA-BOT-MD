	...
	async (message, match) => {
		...
		if (!match) {
			...
			const button = await genButtonMessage(
				...
				'උදාහරණය\nලින්ක් දමන සියල්ලන් ඉවත් කිරීම.,
				'Antilink'
			)
			...
			// 				urlButton: {
			// 					text: 'උදාහරණය',
			// 					url: 'ලින්ක් දමන සියල්ලන් ඉවත් කිරීම.',
			// 				},
			// 			},
			// 			{
			// 				button: {
		...
		if (match == 'on' || match == 'off') {
			if (match == 'off' && !antilink)
				return await message.send('*⚠️ ලින්ක් දැමීම නිසා &mention ඉවත් කෙරේ.*')
		...
		if (match.startsWith('action/')) {
			...
			if (!['delete', 'kick', 'sithu'].includes(action))
