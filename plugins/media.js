...
bot(
	...
	async (message, match) => {
		if (!message.reply_message || !message.reply_message.video)
			return await message.send('*වීඩියෝවකට පිළිතුරු දෙන්න.*')
		if (match === '')
			return await message.send('*උදාහරණය : rotate right|left|flip*')
		...
		} else await message.send('*උදාහරණය : rotate right|left|flip*')
	}
)
...
bot(
	...
	async (message, match) => {
		if (
			...
			return await message.send('*වීඩියෝවකට සහ සිංදුවකට පිළිතුරු දෙන්න.*')
		return await message.send(
...
bot(
	...
	async (message, match) => {
		...
		if (!match) return await message.send('*උදාහරණය : trim 0;30*')
		const [start, duration] = match.split(';')
		if (!start || !duration || isNaN(start) || isNaN(duration))
			return await message.send('*උදාහරණය : trim 10;30*')
		return await message.send(
			await audioCut(
...
bot(
	...
	async (message, match) => {
		if (!message.reply_message || !message.reply_message.video)
			return await message.send('*වීඩියෝවකට පිළිතුරු දෙන්න.*')
		if (!match) return await message.send('*උදාහරණය : trim 10;30*')
		const [start, duration] = match.split(';')
		if (!start || !duration || isNaN(start) || isNaN(duration))
			return await message.send('*උදාහරණය : trim 60;30*')
		return await message.send(
			await videoTrim(
