...
bot(
	{
		pattern: 'kick ?(.*)',
		fromMe: fm,
		desc: 'සමූහයෙන් සාමාජිකයකු ඉවත් කරන්න.',
		type: 'group',
		onlyGroup: true,
	},
	async (message, match) => {
...
bot(
	...
	async (message, match) => {
		...
		if (!match) return await message.send('උදාහරණය : add 91987654321')
...
bot(
	...
	async (message, match) => {
		match = match || message.reply_message.text
		if (!match) return await message.send('*උදාහරණය : info group_invte_link*')
