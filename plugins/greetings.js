	...
	async (message, match) => {
		...
		if (!match) {
			...
			const button = await genButtonMessage(
				...
				'උදාහරණය\nhttps://https://github.com/Sithuwa/SITHUWA-BOT-MD',
				'Welcome'
			)
		...
		if (match == 'on' || match == 'off') {
			if (!welcome) return await message.send('*උදාහරණය : welcome Hi #mention*')
...
bot(
	...
	async (message, match) => {
		...
		if (!match && !welcome)
			return await message.send('*උදාහරණය : goodbye Bye &mention*')
		if (!match) {
			await message.send(welcome.message)
			const onOrOff = welcome && welcome.enabled ? 'off' : 'on'
			const button = await genButtonMessage(
